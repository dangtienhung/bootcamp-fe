...
-- check trong sql ~ if ngôn ngữ lập khác
...
(N'Nguyễn Văn A', '1990-05-15', 1, '2024-05-14', N'IT', 8000000),
...
--> AVG => tính trung bình
SELECT * FROM NHAN_VIEN
SELECT SUM(NHAN_VIEN.LUONG) AS N'TỔNG LƯƠNG'FROM NHAN_VIEN 
SELECT AVG(NHAN_VIEN.LUONG) AS N'TRUNG BÌNH LƯƠNG CỦA NHÂN VIÊN' FROM NHAN_VIEN

--> TÍNH RA TRUNG BÌNH LƯƠNG CỦA TỪNG NGHỀ
SELECT SUM(NHAN_VIEN.LUONG) AS N'TỔNG LƯƠNG', NHAN_VIEN.NGHE_NGHIEP FROM NHAN_VIEN 
GROUP BY NHAN_VIEN.NGHE_NGHIEP
SELECT AVG(NHAN_VIEN.LUONG) AS N'TRUNG BÌNH LƯƠNG CỦA NHÂN VIÊN', NHAN_VIEN.NGHE_NGHIEP FROM NHAN_VIEN
GROUP BY NHAN_VIEN.NGHE_NGHIEP


-- LẤY RA NHỮNG BẠN MỚI LÀM VÀO NGÀY HÔM NAY
SELECT * FROM NHAN_VIEN 

SELECT * FROM NHAN_VIEN 
WHERE DATEDIFF(YEAR, NHAN_VIEN.NGAY_VAO_LAM, GETDATE()) = 0

-- LẤY RA 3 NHÂN VIÊN CŨ NHẤT Ở CTY
-- ASC => LẤY RA MỚI NHẤT GẦN NHẤT
-- DESC => LẤY RA CŨ NHẤT
SELECT * FROM NHAN_VIEN
ORDER BY NHAN_VIEN.NGAY_VAO_LAM ASC
OFFSET 3 ROWS 
FETCH NEXT 3 ROWS ONLY

SELECT * FROM NHAN_VIEN
ORDER BY NHAN_VIEN.NGAY_VAO_LAM DESC
OFFSET 3 ROWS 
...
-- LẤY RA TẤT CẢ TÊN NHÂN VIÊN, GIỚI TÍNH, TÍNH TỐNG TIỀN LƯƠNG CỦA NHÂN VIÊN DỰA VÀO TÊN, GIỚI TÍNH
SELECT NHAN_VIEN.TEN_NV, NHAN_VIEN.GIOI_TINH, SUM(NHAN_VIEN.LUONG) FROM NHAN_VIEN
GROUP BY NHAN_VIEN.TEN_NV, NHAN_VIEN.GIOI_TINH
